...
class AOIAgent:
    ...
    def feature_extract(self, obs):
        ...
        state, i, j, net_map, road_aoi = obs['state'], obs['target'][0], obs['target'][1], obs['net_map'], obs[
            'road_aoi']
        ...
        x = torch.cat(
            [target.unsqueeze(0), state.unsqueeze(0), road_aoi.unsqueeze(0).to(self.device), net_map.to(self.device)],
            dim=0)
        return x.float()
    ...
    def save(self, path):
        ...
        mean_std = torch.stack([self.input_mean, self.input_std], dim=0).detach().cpu().numpy()
    ...
    def load(self, path):
        ...
        self.input_mean, self.input_std = torch.from_numpy(mean).float(), torch.from_numpy(std).float()
